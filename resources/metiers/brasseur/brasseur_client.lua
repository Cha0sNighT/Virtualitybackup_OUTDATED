	...
	AddEventHandler('brasseur:drawMarker', function (boolean)
		...
		Citizen.CreateThread(function()
			while Brasseur_markerBool == true do
				...
				if isInServiceBrasseur and GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), brasseur_blips["Brasserie sans alcool"].x,brasseur_blips["Brasserie sans alcool"].y,brasseur_blips["Brasserie sans alcool"].z, true) <= brasseur_blips["Brasserie sans alcool"].distanceBetweenCoords then
					TriggerServerEvent('brasseur:serverRequest', "GetBieresansalcool")
					Citizen.Wait(brasseur_blips["Brasserie"].defaultTime)
				end
				...
				if isInServiceBrasseur and GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), brasseur_blips["Point de vente sans alcool"].x,brasseur_blips["Point de vente sans alcool"].y,brasseur_blips["Point de vente sans alcool"].z, true) <= brasseur_blips["Point de vente sans alcool"].distanceBetweenCoords then
					TriggerServerEvent('brasseur:serverRequest', "SellBieresansalcool")
					Citizen.Wait(brasseur_blips["Point de vente"].defaultTime)
				end
			end
		end)
		...
		Citizen.CreateThread(function ()
			while Brasseur_markerBool == true do
				...
				if isInServiceBrasseur then
					...
					if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), brasseur_blips["Brasserie sans alcool"].x,brasseur_blips["Brasserie sans alcool"].y,brasseur_blips["Brasserie sans alcool"].z, true) <= brasseur_blips["Brasserie sans alcool"].distanceMarker then
						DrawMarker(1,brasseur_blips["Brasserie sans alcool"].x,brasseur_blips["Brasserie sans alcool"].y,brasseur_blips["Brasserie sans alcool"].z, 0, 0, 0, 0, 0, 0, 2.001, 2.0001, 0.5001, 0, 155, 255, 200, 0, 0, 0, 0)
					end
					...
					if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), brasseur_blips["Point de vente sans alcool"].x,brasseur_blips["Point de vente sans alcool"].y,brasseur_blips["Point de vente sans alcool"].z, true) <= brasseur_blips["Point de vente sans alcool"].distanceMarker then
						DrawMarker(1,brasseur_blips["Point de vente sans alcool"].x,brasseur_blips["Point de vente sans alcool"].y,brasseur_blips["Point de vente sans alcool"].z, 0, 0, 0, 0, 0, 0, 2.001, 2.0001, 0.5001, 0, 155, 255, 200, 0, 0, 0, 0)
					end
...
AddEventHandler('brasseur:getCar', function (source)
	...
	AddEventHandler('brasseur:drawGetBiere', function(qteBase, qteTraite)
		...
		if qteTraite < 64 and qteBase > 0 then
	...
	RegisterNetEvent('brasseur:drawGetBieresansalcool')
	AddEventHandler('brasseur:drawGetBieresansalcool', function(qteBase, qteTraite2)
		if(qteBase == nil) then
			qteBase = 0
		end
		...
		if qteTraite2 < 64 and qteBase > 0 then
			TriggerEvent('player:looseItem',brasseur_ressourceBase, 1)
			TriggerEvent('player:receiveItem',brasseur_ressourceTraite2, 1)
			ClearPrints()
			SetTextEntry_2("STRING")
			AddTextComponentString("~g~Bière sans alcool brassée")
			DrawSubtitleTimed(4500, 1)
		else
			ClearPrints()
			SetTextEntry_2("STRING")
	...
	RegisterNetEvent('brasseur:drawSellBieresansalcool')
	AddEventHandler('brasseur:drawSellBieresansalcool', function (qte2)
		if(qte2 == nil) then
			qte2 = 0
		end

		if qte2 > 0 then
			TriggerEvent('player:looseItem',brasseur_ressourceTraite2, 1)
			local salaire = math.random(brasseur_pay.minimum, brasseur_pay.maximum)
			TriggerServerEvent('mission:completed', salaire)
			ClearPrints()
			SetTextEntry_2("STRING")
			AddTextComponentString("~g~Vous avez vendu des Bières sans alcool")
			DrawSubtitleTimed(2000, 1)
		else
			ClearPrints()
			SetTextEntry_2("STRING")
			AddTextComponentString("~r~Vous n'avez plus de Bière sans alcool")
			DrawSubtitleTimed(2000, 1)
		end
	end)
