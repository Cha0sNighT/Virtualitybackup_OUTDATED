local jailPassword = "smb" --change this password to your liking and don't share it with the criminals ;-)
...
AddEventHandler('chatCommandEntered', function(fullcm)
    ...
    if(cm[1] == "*jailme") then
        ...
        TriggerClientEvent('chatMessage', -1, 'JUGE', { 0, 0, 0 }, GetPlayerName(source) ..' a été condamné à '.. jT ..' secs de prison.')
    ...
    elseif cm[1] == "*jail" then
        if tablelength(cm) > 2 then
            if cm[2] == jailPassword then
                ...
                TriggerClientEvent('chatMessage', -1, 'JUGE', { 0, 0, 0 }, GetPlayerName(tPID) ..' a été condamné à '.. jT ..' secs de prison.')
            else
                print("Incorrect jailPassword entered by ".. GetPlayerName(source))
            end
        end
